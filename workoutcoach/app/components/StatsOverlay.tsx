  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-zinc-300">Set Summary</h3>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-white">{stats.repCount}</p>
        </div>
        <div>
            ...
            <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${
                  confPercent > 70 ? 'bg-green-500' : confPercent > 40 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
                style={{ width: `${confPercent}%` }}
              />
            </div>
            <span className="text-xs text-zinc-400">{confPercent}%</span>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full px-4 py-2.5 bg-white text-black rounded-lg text-sm font-medium hover:bg-zinc-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
